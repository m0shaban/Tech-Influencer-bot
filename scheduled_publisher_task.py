    ...
    async def _publish_post(self, post):
        ...
        try:
            ...
            if post.platform == "telegram":
                ...
                tweet_text = post.caption if (post.link and post.link in post.caption) else f"{post.caption}\n\n{post.link}" if post.link else post.caption
                result = pub.publish_tweet(
                    text=tweet_text,
                    image_url=post.image_url
                )
                ...
                result = {"success": False, "message": f"Unknown platform: {post.platform}"}
