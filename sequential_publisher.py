...
class SequentialPublisher:
    ...
    async def publish_item(
        ...
        print(f"📱 Platforms: {len(publishing_order)} ({', '.join([p['platform'] for p in publishing_order])})")
        print(f"{'='*60}\n")
        ...
        for idx, platform_config in enumerate(publishing_order, 1):
            ...
            try:
                ...
                if enable_cta and published_urls:
                    print(f"🔗 Injecting CTAs from {len(published_urls)} previous platforms...")
                    content = inject_ctas(content, platform, brand_name, published_urls)
                
                # Add cross-pollination snippet (10% of posts)
                if add_cross_pollination and idx == len(publishing_order):  # Only on last platform
        ...
        print(f"📊 Published to {len(published_urls)}/{len(publishing_order)} platforms")
        print(f"{'='*60}\n")
