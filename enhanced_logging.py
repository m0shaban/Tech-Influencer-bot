...
def get_publish_logger() -> logging.Logger:
    ...
    if not any(isinstance(h, RotatingFileHandler) and PUBLISH_LOG.name in str(h.baseFilename) 
...
def get_ai_logger() -> logging.Logger:
    ...
    if not any(isinstance(h, RotatingFileHandler) and AI_LOG.name in str(h.baseFilename)
               for h in logger.handlers):
        handler = RotatingFileHandler(
            AI_LOG,
            maxBytes=5*1024*1024,
