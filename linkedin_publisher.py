...
def test_linkedin_connection() -> bool:
    ...
    try:
        ...
        url = f"{publisher.base_url}/me"
        response = requests.get(url, headers=publisher.headers, timeout=10)
        response.raise_for_status()
        data = response.json()
        print(f"✅ LinkedIn connected: {data.get('localizedFirstName')} {data.get('localizedLastName')}")
        return True
