...
class SystemTester:
    ...
    def test_config(self) -> Dict[str, Any]:
        ...
        for brand_key, brand_cfg in brands.items():
            ...
            required = ["display_name", "language", "system_prompt", "feeds", "platforms", "channel_id"]
            for field in required:
                if field in brand_cfg:
                    ...
                    if field == "feeds":
                        ...
                        print_test(f"    {field}", True, f"Enabled: {', '.join(enabled)}")
    ...
    async def test_ai_providers(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print_test("Loaded keys", len(manager.groq_keys) > 0, f"{len(manager.groq_keys)} keys")
            
            print_info("\nNVIDIA API Keys:")
            print_test("Loaded keys", len(manager.nvidia_keys) > 0, f"{len(manager.nvidia_keys)} keys")
            ...
            try:
                ...
                client = OpenAI(
                    ...
                    api_key=manager.groq_keys[0] if manager.groq_keys else os.getenv("GROQ_API_KEY")
                )
                ...
                if response.choices:
                    print_test("Groq API call", True, f"Response: {response.choices[0].message.content[:30]}")
                    results["passed"] += 1
                else:
        ...
        try:
            ...
            for brand_key, brand_cfg in brands.items():
                ...
                try:
                    ...
                    if post:
                        print_test(f"Fetch post", True, f"'{post.get('title', 'Untitled')[:40]}...'")
                        results["passed"] += 1
                    else:
                        print_test(f"Fetch post", False, "No posts available (might be duplicate filter)")
                        results["warnings"] += 1
                except Exception as e:
            ...
            for brand_key, brand_cfg in brands.items():
                ...
                try:
                    ...
                    if result:
                        ...
                        for field in ["telegram_post", "facebook_post", "blog_content_md", "discord_msg"]:
                        ...
                        if fields_ok:
                            print_test(f"Content generation", True, f"Generated: {', '.join(fields_ok)}")
                            results["passed"] += 1
                            
                            # Show sample
                            sample_field = fields_ok[0]
                            sample = result[sample_field][:100].replace('\n', ' ')
                            print(f"       {Colors.CYAN}Sample: {sample}...{Colors.END}")
                        else:
        ...
        try:
            ...
            for brand_key, brand_cfg in brands.items():
                ...
                try:
                    ...
                    if enabled:
                        print_test(f"Publisher init", True, f"Enabled: {', '.join(enabled)}")
                        results["passed"] += 1
                    else:
    ...
    async def test_image_generator(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                print_test("Arabic font loaded", True, f"Font: {ArabicFontManager._picked_name}")
                results["passed"] += 1
            except Exception as e:
    ...
    async def test_publishing_pipeline(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                print_test("Publishers init", True, f"Enabled: {platform_publisher.enabled_platforms}")
                results["passed"] += 1
            except Exception as e:
    ...
    def print_summary(self):
        ...
        for test_name, result in self.results.items():
            ...
            print(f"  {status} {test_name}: {passed} passed, {failed} failed, {warnings} warnings")
        
        print(f"\n{Colors.BOLD}{'='*60}{Colors.END}")
        print(f"  {Colors.BOLD}TOTAL: {total_passed} passed, {total_failed} failed, {total_warnings} warnings{Colors.END}")
